#' @export UPDATE_lmpEffectMatrices
...
UPDATE_lmpEffectMatrices <- function(resLmpModelMatrix, SS = TRUE, contrastList = NA) {
  #######update Antoine###############
  # Create the variance percentages plots
  # As the code is used twice in the main function, I have created a separate function
  ...
  if(model == "lmm"){
    ...
    if(length(responseWithWarning) != 0 ){
      ...
      warning("The model estimates for these response(s) are biased.")
    }
    ...
    # Fixed effect
    varMfix <- c()
    if(length(effectMatrices) != 0){
      for (i in 1:length(effectsNamesUnique)){
        #  variance of parameters values (population)
        varMfix <- rbind(varMfix, (apply(effectMatrices[[i]], 2, var) *(n - 1) / n))
      }
      rownames(varMfix) <- effectsNamesUnique
    }
    
    # all together
    varComp <- rbind(varMfix, varMrand)
    colnames(varComp) <- colnames(parameters)
    
    # Percent var explicate by each effect
    varComponentsAbs <- rowSums(varComp)
    variationPercentages <- varComponentsAbs*100/sum(varComponentsAbs)
    names(variationPercentages) <- rownames(varComp)
    # Remove intercept
    variationPercentages <- variationPercentages[-1]
    
    varPercentagesPlot <- createVarPercentagesPlot(variationPercentages)
    ...
    resLmpEffectMatrices <- list(
      ...
      MM_full = MM_full,
      varComponentsAbs = varComponentsAbs,
      variationPercentages = variationPercentages,
      varPercentagesPlot = varPercentagesPlot
    )
