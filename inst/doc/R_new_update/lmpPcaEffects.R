...
#'  \item{\code{varComponentsAbs}}{A vector with the absolute variance for each model effect If linear mixed model.}
...
#'
#'   \describe{
#'   \item{ASCA}{PCA is applied directly on each pure effect matrix \eqn{\hat{\mathbf{M}}_g, g=1...F+R}.}
...
#'  }
#'  If we are in the case of APCA or ASCA-E, all effect matrices are increased by the residuals matrix.
#'
#' @examples
#' data("UCH")
...
UPDATE_lmpPcaEffects <- function(resLmpEffectMatrices, method = c("ASCA", "APCA", "ASCA-E"), 
  ...
  if (method == "ASCA") {
    ...
    if(model == "lmm"){
      EffectMatGLMR <- resLmpEffectMatrices$effectMatricesR
      EffectMatGLMR <- lapply(EffectMatGLMR, function(x) x +
  ...
  if (method == "ASCA-E") {
    ...
    if(model == "lmm"){
      for (i in 1:(pR-1)) {
        resLmpPcaEffectsR[[i]]$scores[, 1:5] <- (EffectMatGLMR[[i]] +
                                                  EffectMatGLMR[[pR]]) %*% resLmpPcaEffectsR[[i]]$loadings[, 1:5]
      }
    }
