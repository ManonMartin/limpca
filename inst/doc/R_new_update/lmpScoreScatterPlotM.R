#' @export UPDATE_lmpScoreScatterPlotM
...
UPDATE_lmpScoreScatterPlotM <- function(resLmpPcaEffects,
                                 effectNames = NULL,
                                 PCdim = NULL,
                                 modelAbbrev = FALSE, ...) {
  ...
  if (allEffect == FALSE) {
    if (length(PCdim) != length(effectNames)) {
      stop("length(PCdim) differs from length(effectNames)")
    }
  }
