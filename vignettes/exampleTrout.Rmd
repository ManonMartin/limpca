...
The trout dataset contains 3 elements : a matrix `outcomes` with 72 observations of 15 response variables, a formula for the GLM model used and a `design` matrix with 72 observations and 3 explanatory variables.
...
DrawScores(ResPCA, type.obj = "PCA", drawNames = TRUE, createWindow = F, 
    main = "Reponse matrix score plot", color = trout$design$Exposure.Cd, pch = trout$design$Treatment, 
    axes = c(1, 2), size = 2.5) + ggplot2::scale_color_discrete(name = "Exposure.Cd") + ggplot2::scale_shape_discrete(name = "Treatment")

DrawScores(ResPCA, type.obj = "PCA", drawNames = TRUE, createWindow = F, 
    main = "Reponse matrix score plot", color = trout$design$Treatment, pch = trout$design$Day, 
    axes = c(1, 2), size = 2.5) + ggplot2::scale_color_discrete(name = "Treatment") + ggplot2::scale_shape_discrete(name = "Day")
...
PlotScoresMatrix(ResASCA,
                 ...
                 vec.colorup = c("brown1","chartreuse3","cyan3", "darkorchid1"),
                 varname.colordown  = "Exposure.Cd",
                 vec.colordown = c("darkorange","midnightblue"),
                 varname.pchup="Day",
                 vec.pchup = c(4,16,2),
                 varname.pchdown="Day",
                 vec.pchdown = c(1,3,5))
...
plot(ResAPCA$Residuals$scores, pch = NA_integer_)
text(ResAPCA$Residuals$scores, labels = rownames(outcomes), cex = 0.5)
abline(h=0, v=0)
```
