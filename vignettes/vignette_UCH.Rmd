...
The contributions from each effect and their principal components are estimated and reported in tables with the function `lmwContributions()`. 
...
The loadings are here represented on a line plot with the function `lmwLoadingPlot()` to conveniently compare them with the original spectral profiles. 

```{r}
all_loadings_pl <- lmwLoadingPlot(resASCA, 
...
lmwPcaScatterPlot(resASCA, what = "loadings", 
                  effectNames = "Hippurate",color="test",
                  design=data.frame(test = rep(c("1","2"), 
                                               length.out = nrow(resASCA$Hippurate$loadings))))
lmwPcaScatterPlot(resASCA, what = "loadings", 
                  effectNames = "Hippurate",
                  design=NULL)

lmwPcaScatterPlot(resASCA, what = "scores", 
                  effectNames = "Hippurate", 
...
plotLine(t(resASCA$Time$scores),  
         rows = 1, type = c("p"),xaxis_type = "character")

tim_scores_pl <- lmwScorePlot(resASCA, effectNames = "Time", color = "Time", shape = "Time")
...
grid.arrange(tim_scores_pl,hiptim_loadings_pl, ncol=2)
...
hiptimInter_loadings_pl <- all_loadings_pl$`Hippurate:Time` + time_peaks + hip_peaks

grid.arrange(hiptimInter_scores_pl,hiptimInter_loadings_pl, ncol=2)

```

Finally we can represent the scores with a matrix of plots with `lmwScoreScatterPlotM()`. This graph allows to observe multiple variables simultaneously.
...
Only possible with the ASCA method
...
The ASCA-E method performs PCA on the effect matrices then adds the residuals to compute the augmented scores. The same functions are used.
