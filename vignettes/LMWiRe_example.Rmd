...
The package is actually in its development stage and is available on GitHub at the url:<https://github.com/bgovaerts/LMWiRe>. It can be installed via the `devtools::install_github()` function. The package needs to be loaded afterwards.


```{r Install, results=FALSE, message=FALSE}

# devtools::install_github("bgovaerts/LMWiRe", force = TRUE)
...
pcaScorePlot(resPcaBySvd = ResPCA, axes = c(1,2), 
             title = "PCA score plot: PC1 and PC2", 
             design = UCH$design,
...
pcaScorePlot(resPcaBySvd = ResPCA, axes = c(3,4), 
             title = "PCA score plot: PC3 and PC4", 
             design = UCH$design,
...
p2 <- pcaLoadingPlot(resPcaBySvd = ResPCA, axes = c(1,2), 
               title = "PCA loading plot UCH", xlab = "ppm", ylab = "Intensity")
...
resLmwBootstrapTests = lmwBootstrapTests(resLmwEffectMatrices = resLmwEffectMatrices, nboot=1000)
...
lmwScoreScatterPlotM(resASCA,
                     ...
                     vec.pchdown = c(1,3), title = "ASCA score scatterplot matrix")
```
...
lmwScoreScatterPlotM(resASCAE,
                     ...
                     vec.pchup = c(4,16,2),
                     varname.pchdown="Time",
                     vec.pchdown = c(1,3), title = "ASCA-E score scatterplot matrix")
```
...
lmwScoreScatterPlotM(resAPCA,
                     ...
                     vec.pchup = c(4,16,2),
                     varname.pchdown="Time",
                     vec.pchdown = c(1,3), title = "APCA score scatterplot matrix")
```
