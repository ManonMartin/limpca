...
lmpEffectMatrices <- function(resLmpModelMatrix, SS = TRUE, contrastList = NA) {
  ...
  if(model == "lmm"){
    ...
    # Allows you to display a specific message error
      res.lmer[[i]] <- withCallingHandlers({
        if(length(contrastsSumList) != 0){
         res.lmer[[i]] <- lme4::lmer(fmla[[i]], data = data_full, contrasts = contrastsSumList, control = lme4::lmerControl(optimizer = "bobyqa"))
        } else {
         res.lmer[[i]] <- lme4::lmer(fmla[[i]], data = data_full, control = lme4::lmerControl(optimizer = "bobyqa"))
        }
      },
    ...
    Zt <- lme4::getME(MM_full[[1]], "Zt")
    modelMatrixR <- as.matrix(Matrix::t(Zt))
  ...
  resLmpSS <- lmpSS(resLmpEffectMatrices, L, model)
