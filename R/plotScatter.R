...
plotScatter <- function(Y, xy, design = NULL, color = NULL,
  ...
  if (!is.null(design)) {
    if (length(points_labs) != nrow(design) & !is.null(points_labs)) {
      stop("length of points_labs is different than the
  ...
  if (drawShapes != "none") {
    if (drawShapes == "ellipse") {
      ...
      df_tot_centr <- df_tot %>% dplyr::left_join(centroids,
        by = unique(c(color, shape)),
        suffix = c("", ".centroid")
      )
